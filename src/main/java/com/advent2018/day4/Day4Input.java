...
import static com.advent2018.day4.Day4Input.ActionType.*;
import static java.lang.Integer.parseInt;
import static java.util.Arrays.asList;

public final class Day4Input {

    private static final Pattern START_SHIFT_PATTERN = Pattern.compile("^\\[(\\d+)-(\\d+)-(\\d+) (\\d+):(\\d+)\\] Guard #(\\d+) begins shift$");
    private static final Pattern SLEEP_PATTERN = Pattern.compile("^\\[(\\d+)-(\\d+)-(\\d+) (\\d+):(\\d+)\\] falls asleep$");
    private static final Pattern WAKE_PATTERN = Pattern.compile("^\\[(\\d+)-(\\d+)-(\\d+) (\\d+):(\\d+)\\] wakes up$");

    public enum ActionType {
        START_SHIFT, SLEEP, WAKEUP
    }

    public static List<SleepingSchedule> getSleepingSchedules() {

        List<SleepingSchedule> schedules = new ArrayList<>();
        List<String> actionsString = asList(INPUT.split("\\n"));
        System.err.println("Found " + actionsString.size() + " actions");

        List<Action> actions = getOrderedActions(actionsString);

        Matcher matcher;
        SleepingSchedule sleepingSchedule = null;
        for(int i = 0; i < actions.size(); i++) {
            Action action = actions.get(i);

            if(action.getActionType() == START_SHIFT) {
                if(sleepingSchedule != null) {
                    schedules.add(sleepingSchedule);
                }
                matcher = getStartShift(action.getActionString());
                matcher.matches();
                sleepingSchedule = startSleepingSchedule(matcher);
                continue;
            }

            if(action.getActionType() == SLEEP) {
                matcher = getStartSleep(action.getActionString());
                matcher.matches();
                sleepingSchedule = startSleeping(sleepingSchedule, matcher);
                continue;
            }

            if(action.getActionType() == WAKEUP) {
                matcher = getWakesup(action.getActionString());
                matcher.matches();
                sleepingSchedule = wakesUp(sleepingSchedule, matcher);
                if(i == actions.size() - 1) {
                    schedules.add(sleepingSchedule);
                }
                continue;
    ...
    private static List<Action> getOrderedActions(List<String> actions) {
        Matcher m;
        List<Action> parsedActions = new ArrayList<>();
        for(String stringAction : actions) {
            m = getStartShift(stringAction);
            if(m.matches()) {
                parsedActions.add(new Action(getDateFromAction(m), stringAction, START_SHIFT));
                continue;
            }

            m = getStartSleep(stringAction);
            if(m.matches()) {
                parsedActions.add(new Action(getDateFromAction(m), stringAction, SLEEP));
                continue;
            }

            m = getWakesup(stringAction);
            if(m.matches()) {
                parsedActions.add(new Action(getDateFromAction(m), stringAction, WAKEUP));
                continue;
            }
        }

        Collections.sort(parsedActions, Comparator.comparing(Action::getActionDate));
        return parsedActions;
    }

    private static DateTime getDateFromAction(Matcher matcher) {
        int year = 2017;
    ...
    private static SleepingSchedule startSleepingSchedule(Matcher matcher) {
        String guardId = matcher.group(6);

        SleepingSchedule s = new SleepingSchedule();
        s.setGuard(guardId);

        return s;
    }

    private static SleepingSchedule startSleeping(SleepingSchedule sleepingSchedule, Matcher matcher) {
...
//        d.setMonth(month);
//        d.setDate(day);
        DateTime d = getDateFromAction(matcher);
        sleepingSchedule.setDateTime(d);

        int startMinute =  parseInt(matcher.group(5));
        sleepingSchedule.setMinuteStartSleeping(startMinute);
        return sleepingSchedule;
    }

    private static SleepingSchedule wakesUp(SleepingSchedule sleepingSchedule, Matcher matcher) {
        int wakeUpMinute =  parseInt(matcher.group(5));
        sleepingSchedule.setMinuteWakesUp(wakeUpMinute);
        return sleepingSchedule;
    }

    private static Matcher getStartShift(String action) {
        return START_SHIFT_PATTERN.matcher(action);
    }

    private static Matcher getStartSleep(String action) {
        return SLEEP_PATTERN.matcher(action);
    }

    private static Matcher getWakesup(String action) {
        return WAKE_PATTERN.matcher(action);
    }
