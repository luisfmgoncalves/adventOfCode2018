...
public final class Day10Input {
    ...
    public static List<Point> getPointList() {
        return stream(INPUT.split("\\n"))
                .map(pointStr -> toPoint(pointStr))
                .collect(Collectors.toList());
    }

    private static Point toPoint(String pointStr) {
        Matcher matcher = POINT_PATTERN.matcher(pointStr.replaceAll(" ",""));
        matcher.matches();
        if(matcher.matches()) {
            return new Point(parseInt(matcher.group(1).trim()),
                             parseInt(matcher.group(2).trim()),
                             parseInt(matcher.group(3).trim()),
                             parseInt(matcher.group(4).trim()));
        }
        return null;
    }
